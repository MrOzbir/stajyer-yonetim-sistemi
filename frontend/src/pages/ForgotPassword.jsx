    ...
    return (
        <div className="min-h-screen flex items-center justify-center bg-night p-4">
            <div className="w-full max-w-md bg-panel border border-edge rounded-2xl p-6 shadow-2xl">
                <Link to="/login" className="inline-flex items-center gap-1 text-xs text-snow-muted hover:text-snow mb-4 transition-colors">
                ...
                <p className="text-xs text-snow-muted mb-6">
                ...
                {!message && (
                    <form onSubmit={handleSubmit} className="space-y-4">
                        <div>
                            <label className="block text-xs font-semibold text-snow-muted mb-1">E-posta Adresi</label>
                            <div className="relative">
                                <Mail className="absolute left-3 top-3 text-snow-faint" size={16} />
                                <input
                                    type="email"
                                    required
                                    value={email}
                                    onChange={(e) => setEmail(e.target.value)}
                                    className="w-full bg-overlay border border-edge rounded-lg py-2.5 pl-9 pr-3 text-xs text-snow outline-none focus:border-brand"
