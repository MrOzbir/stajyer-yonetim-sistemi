...
// --- YARDIMCI İLİŞKİLER (archives yerine archiveEntries yapıldı) ---
const INTERN_INCLUDE = {
    internProfile: true,
    department: true,
    tasksReceived: { select: { id: true, status: true, deadline: true } },
    logs: { select: { loginTime: true, logoutTime: true } },
    archiveEntries: { select: { id: true, date: true, mood: true, sentimentScore: true } },
    aiReports: {
...
function buildInternStats(intern) {
    ...
    const archives = intern.archiveEntries || [];
    ...
    logs.forEach(log => {
        if (log.logoutTime) {
            totalWorkedMinutes += Math.round((new Date(log.logoutTime) - new Date(log.loginTime)) / 60000);
        } else {
            isActiveNow = true;
        }
...
exports.getInternById = async (internId) => {
    const intern = await prisma.user.findUnique({
        ...
        include: {
            ...
            archiveEntries: { orderBy: { date: 'desc' }, take: 30 },
